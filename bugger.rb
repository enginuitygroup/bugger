...
def bugger_watch_list
  JSON.parse(File.read("config.json"))["watch_list"]
end

def bugger_personal_access_token
  JSON.parse(File.read("config.json"))["personal_access_token"]
end
...
def matches_found? webhook_payload, sha_of_last_commit
  ...
  regexes = Regexp.union bugger_watch_list
  final_regex = /\+.*#{regexes}/
  number_of_matches = 0

  diff = JSON.parse(Faraday.get(path, {access_token: bugger_personal_access_token}).body, symbolize_names: true)
...
def update_status match_found, statuses_url, sha_of_last_commit
  ...
  post_response = Faraday.post(statuses_url.gsub("{sha}", sha_of_last_commit)) do |req|
    ...
    req.params = {access_token: bugger_personal_access_token}
